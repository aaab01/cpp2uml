...
def found_class(node):
	...
	for c in node.get_children():
		...
		if clang.cindex.CursorKind.CLASS_DECL == c.kind:
			found_class(c);

		if clang.cindex.CursorKind.CXX_METHOD == c.kind:
			# print 'CXX_METHOD :: %s, line %d' % (tripfun(val), c.location.line)
			obj.addFuncion(tripfun(val))

		if clang.cindex.CursorKind.CONSTRUCTOR == c.kind:
			# print 'CONSTRUCTOR :: %s, line %d' % (tripfun(val), c.location.line)
			obj.addFuncion(tripfun(val))

		if clang.cindex.CursorKind.DESTRUCTOR == c.kind:
			# print 'DESTRUCTOR :: %s' % (tripfun(val))
			obj.addFuncion(tripfun(val))

		if clang.cindex.CursorKind.FIELD_DECL == c.kind:
			# print 'FIELD_DECL :: %s' % (val)
			obj.addMember(val);

		if clang.cindex.CursorKind.CXX_ACCESS_SPEC_DECL == c.kind:
...
def find_typerefs(node):
	...
	if clang.cindex.CursorKind.CLASS_DECL == node.kind or clang.cindex.CursorKind.STRUCT_DECL == node.kind:
		found_class(node)
	else:
		for c in node.get_children():
			find_typerefs(c)
